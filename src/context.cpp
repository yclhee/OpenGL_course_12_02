...
void Context::Render() {
    ...
    if (ImGui::Begin("ui window")) {
        ...
        ImGui::DragFloat("gamma", &m_gamma, 0.0f, 0.0f, 2.0f);
        ImGui::Separator();
    ...
    // m_postProgram->Use();
    // m_postProgram->SetUniform("transform", glm::scale(glm::mat4(1.0), glm::vec3(2.0f, 2.0f, 1.0f)));
...
void Context::DrawScene(const glm::mat4& view,
    ...
    program->SetUniform("modelTransform", modelTransform);
    m_box2Material->SetToProgram(program);
    m_box->Draw(program);  // 공중에 떠 있는 새로운 박스 추가
}
